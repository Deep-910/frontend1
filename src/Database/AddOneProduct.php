...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['img1']) && isset($_FILES['img2']) && isset($_FILES['img3']) && isset($_FILES['img4'])) {
  ...
  //  {
    ...
    if ($upload1 && $upload2 && $upload3 && $upload4) {
        $query = "INSERT INTO oneproduct (pname, category, brand, description, img1, img2, img3, img4, date,endDate,p_price,discount,p_rate) VALUES ( ?, ?,?, ?,  ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssssssssii",  $pname, $category, $brand, $description, $img1, $img2, $img3, $img4, $date,$endDate,$price, $discount,$rate);
        $res = $stmt->execute();
